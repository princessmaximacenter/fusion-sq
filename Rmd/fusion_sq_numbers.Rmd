---
title: "Fusion-sq  - Cohort analysis"
output:
  html_document:
    df_print: paged
    toc: true
...
uq_gene_pairs_df %>% filter(fusion_name=="DUS2--NFATC3")
fga_anno = read.table("~/fusion_sq/fga_anno.tmp.tsv",sep="\t",header=T)

patient_labels_lookup = cohort[,c("patient_id","patient_label")]

primary_group_shorthand_table = read.table(paste0("/Users/ianthevanbelzen/Documents/resources/diagnoses_icdo3_prigroup.mapping.tsv"),header=T,sep = "\t") %>% dplyr::rename(primary_group_shorthand_label = primary_group_label)
...
#cohort_results = cohort_results %>% mutate(patient_clinrel_fusion = patient_id %in% filter(cohort,fusion_status)$patient_id)
...
recurrence_table = read.table(recurrence_table_path,header=T,sep="\t")
...
uq_fusions_df = uq_fusions_df %>% rowwise() %>% mutate(gup_chromosome = strsplit(gup_sv_merged_coordinate,":",fixed=T)[[1]][1],
                                          gdw_chromosome = strsplit(gdw_sv_merged_coordinate,":",fixed=T)[[1]][1]) %>% as.data.frame()

fusion_bp_in_cn_amplified = uq_fusions_df  %>%  filter_at(vars(contains("copy_ratio")), any_vars(.>1.58)) 
fusion_bp_in_cn_amplified %>% uq_fusions() 

uq_fusions_df = uq_fusions_df %>% mutate(flag_in_cn_amplified = patient_fusion_sv %in% fusion_bp_in_cn_amplified$patient_fusion_sv)
...
```{r}
 ...
 cohort_report %>% filter(precise_confident & somatic_variant & rna_tools=="fusioncatcher, starfusion") %>% 
    select(c("rt_switch_bp","rt_early_bp","rt_late_bp")) %>% colSums(na.rm = T)
  
  cohort_report %>% filter(precise_confident & germline_variant & rna_tools=="fusioncatcher, starfusion") %>% 
    select(c("rt_switch_bp","rt_early_bp","rt_late_bp")) %>% colSums(na.rm = T)
  
  cohort_report %>% filter(precise_confident & low_af & rna_tools=="fusioncatcher, starfusion") %>% 
    select(c("rt_switch_bp","rt_early_bp","rt_late_bp")) %>% colSums(na.rm = T)
  
  
  cohort_report %>% filter(precise_confident & rna_tools=="fusioncatcher, starfusion") %>% 
    select(c("rt_switch_bp","rt_early_bp","rt_late_bp")) %>% colSums(na.rm = T)
  
  cohort_report %>% filter(precise_confident & rna_tools=="fusioncatcher, starfusion") %>% 
    select(c("rt_switch_bp","rt_early_bp","rt_late_bp")) %>% colMeans(na.rm = T)
...
## Patient counts and distribution over cancer types

```{r results="asis"}

cat(paste0("* ","Cohort size: ",uq_patients(cohort), 
"\n"))

cat(paste0("* ","Tumor types: ",length(unique(filter(cohort,!is.na(tumor_type))$tumor_type)),"\n"))
cat(paste0("* ","Cancer type groups: ",length(unique(filter(cohort,!is.na(primary_group))$primary_group)),"\n"))

cat(paste0("* ","Patient missing tumor type: ",uq_patients(filter(cohort,is.na(tumor_type))),"\n"))
cat(paste0("* ","Patient missing cancer type group: ",uq_patients(filter(cohort,is.na(primary_group))),"\n"))

cat(paste0("* ","Cancer type groups NOT represented: ",paste(primary_group_shorthand_table %>% filter(!primary_group %in% cohort$primary_group) %>% select(primary_group_shorthand_label)),"\n"))

cat(paste0("* ",
  uq_patients(filter(cohort,fusion_status)),
  " patients with clinically validated gene fusion",
"\n"))

cat(paste0("* ","Tumor types, patients with clin.rel fusion: ",length(unique(filter(cohort,fusion_status)$tumor_type)),"\n"))
cat(paste0("* ","Cancer type groups, patients with clin.rel  fusion: ",length(unique(filter(cohort,fusion_status)$primary_group)),"\n"))
cat(paste0("* ","Cancer type groups, patients with clin.rel fusion: ",paste0(unique(filter(cohort,fusion_status)$primary_group_shorthand_label),collapse="; "),"\n"))

cat(paste0("* ",
  uq_patients(filter(cohort,is.na(fusion_status))),
  " patients with unknown fusion status",
"\n"))

cat(paste0("* ",
  uq_patients(filter(cohort,!fusion_status)),
  " patients without clinically relevant fusions",
"\n"))

cat(paste0("* ","Tumor types, patients NO clin.rel fusion: ",length(unique(filter(cohort,!fusion_status&!is.na(tumor_type))$tumor_type)),"\n"))
cat(paste0("* ","Cancer type groups, patients NO clin.rel fusion: ",length(unique(filter(cohort,!fusion_status &!is.na(primary_group))$primary_group)),"\n"))

```

So we have patients without validated clin.rel fusion in across the primary cancer groups
...
cat(paste0("  * precise+confident: ",
           uq_fusions(filter(cohort_report,high_confidence_rna_dna)),"\n"))
...
filter(cohort_report,high_confidence_rna_dna) %>% uq_fusions()
filter(cohort_report,high_confidence_rna_dna) %>% uq_patients()

filter(cohort_report,high_confidence_rna_dna&somatic_variant) %>% uq_fusions()

filter(uq_fusions_hc,high_confidence_rna_dna&!patient_clinrel_fusion&anno_has_onco_or_tsg&somatic_variant) %>% uq_fusions()
...
,"\n"))
cat(paste0("  * ","median high conf ts, non-zero, !patient_clinrel_fusion: ",
median(filter(patient_fusion_cnt_table,!patient_clinrel_fusion&somatic_hc_cnt>0)$somatic_hc_cnt)
,"\n"))

cat(paste0("  * !patient_clinrel_fusion (==excl validation set)  ",
           uq_fusions(filter(cohort_report,somatic_variant  & !patient_clinrel_fusion)),"\n"))

cat(paste0("  * !patient_clinrel_fusion & precise_confident: ",uq_fusions(filter(cohort_report,somatic_variant&!patient_clinrel_fusion&precise_confident)),"\n"))

cat(paste0("  * !patient_clinrel_fusion & precise_confident & onco/tsg: ",uq_fusions(filter(cohort_report,somatic_variant&precise_confident&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion)),"\n"))

cat(paste0("  * !patient_clinrel_fusion & precise_confident & !onco/tsg & anno_cancer_chimera: ",uq_fusions(filter(cohort_report,somatic_variant&precise_confident&!(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion&anno_cancer_chimera)),"\n"))


cat(paste0("  * !patient_clinrel_fusion & 2 tools: ",uq_fusions(filter(cohort_report,somatic_variant&!patient_clinrel_fusion& grepl(",",tools_any_wgs))),"\n"))

cat(paste0("  * !patient_clinrel_fusion & location precise: ",uq_fusions(filter(cohort_report,somatic_variant&!patient_clinrel_fusion&location_precise)),"\n"))
...
```{r results="asis"}
cat(paste0("* tumor specific !patient_clinrel_fusion & precise_confident: ",
filter(uq_fusions_hc,somatic_variant&!patient_clinrel_fusion) %>%nrow(),"\n"))
...
cat(paste0("  * !patient_clinrel_fusion (==excl validation set)  ",
           uq_uq_fusions(filter(cohort_report,somatic_variant  & !patient_clinrel_fusion)),"\n"))

cat(paste0("  * !patient_clinrel_fusion & precise_confident: ",uq_uq_fusions(filter(cohort_report,somatic_variant&!patient_clinrel_fusion&precise_confident)),"\n"))

cat(paste0("  * !patient_clinrel_fusion & precise_confident & onco/tsg: ",uq_uq_fusions(filter(cohort_report,somatic_variant&precise_confident&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion)),"\n"))
...
cat(paste0("  * !patient_clinrel_fusion: ",uq_patients(filter(cohort_report,somatic_variant&!patient_clinrel_fusion)),"\n"))
cat(paste0("  * !patient_clinrel_fusion & precise_confident: ",uq_patients(filter(cohort_report,somatic_variant&!patient_clinrel_fusion&precise_confident)),"\n"))
cat(paste0("  * !patient_clinrel_fusion & precise_confident & onco/tsg: ",uq_patients(filter(cohort_report,somatic_variant&precise_confident&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion)),"\n"))
...
```{r}
fusions_unkn_sign = cohort_report %>% filter(!clinically_validated&!clinically_validated_reciprocal&somatic_variant &precise_confident & patient_clinrel_fusion) 
...
cat(paste0("* ","Fusions validated by WGS vs fusion overview: ",
           uq_fusions(filter(cohort_report,patient_clinrel_fusion)), " (",round(
           uq_fusions(filter(cohort_report,patient_clinrel_fusion))/uq_fusions(filter(fusion_overview,patient_clinrel_fusion)),4),")\n"))


cat(paste0("* ","Fusions validated by WGS as tumor specific, precise & confident SV vs fusion overview: ",
           uq_fusions(filter(cohort_report,patient_clinrel_fusion & somatic_variant & precise_confident)), " (",round(
           uq_fusions(filter(cohort_report,patient_clinrel_fusion & precise_confident & somatic_variant))/uq_fusions(filter(fusion_overview,patient_clinrel_fusion)),4),")\n"))
...
cat(paste0("* ","Fusions validated by WGS as tumor specific: ",
           uq_fusions(filter(cohort_report,!patient_clinrel_fusion & somatic_variant)), " (",round(
           uq_fusions(filter(cohort_report,!patient_clinrel_fusion & somatic_variant))/uq_fusions(filter(fusion_overview,!patient_clinrel_fusion)),4),")\n"))
...
uq_fusions(filter(cohort_report,precise_confident&somatic_variant&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion))

print("which onco/tsg fusions are filtered out by precise confident filter?")
cohort_report %>% filter(!precise_confident&somatic_variant&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion) %>% select(patient_fusion,gup_location,gdw_location,tools_any_wgs)
...
```{r results="asis"}
cat(paste0("* Mean size somatic: ",mean(filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$svlen,na.rm=T)%>% round(.),"\n "))
...
```{r}
mean(filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$svlen,na.rm=T)/mean(filter(uq_gene_pairs_hc,variant_type=="germline")$svlen,na.rm = T)
```

```{r}
wilcox.test(filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$svlen,
filter(uq_gene_pairs_hc,variant_type=="germline")$svlen,alternative = "greater")

wilcox.test(filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$svlen,
filter(uq_gene_pairs_hc,variant_type!="tumor_specific")$svlen,alternative = "greater")
...
print("somatic")
summary(filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$svtype_label)
print("somatic, clinrel")
summary(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&clinically_validated)$svtype_label)
print("somatic, no clinrel")
summary(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&!clinically_validated)$svtype_label)

filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&!clinically_validated) %>% nrow()
...
cat(paste0("* Mean size somatic HC: ",mean(filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$svlen,na.rm=T)%>% round(.),"\n "))
cat(paste0("* Mean size somatic LC: ",mean(filter(uq_gene_pairs_lc,variant_type=="tumor_specific")$svlen,na.rm=T)%>% round(.),"\n "))
...
print("somatic")
summary(filter(uq_gene_pairs_lc,variant_type=="tumor_specific")$svtype_label)
print("germline")
...
cat(paste0("* Mean size somatic: ",mean(filter(uq_fusions_hc,variant_type=="tumor_specific")$svlen,na.rm=T)%>% round(.),"\n "))
...
cat(paste0("* Mean size somatic excl the 36 clin rel: ",mean(filter(uq_fusions_hc,variant_type=="tumor_specific"&!clinically_validated)$svlen,na.rm=T)%>% round(.),"\n "))
...
```{r}
wilcox.test(filter(uq_fusions_hc,variant_type=="tumor_specific")$svlen,
...
```{r}
patient_fusion_cnt_table %>% filter(somatic_low_af_hc_cnt>=perc_somatic_low_af["95%"]) %>% select(somatic_low_af_hc_cnt,somatic_hc_cnt,fga,primary_group_shorthand_label,patient_id) %>% arrange(-somatic_low_af_hc_cnt) %>% left_join(cohort[,c("patient_id","tumor_type_label")],by="patient_id")
...
low_af_selection %>% uq_fusions()
low_af_selection %>% uq_uq_fusions()
#no recurrrence
...
cnt_per_cytoband  = uq_fusions_hc %>% 
  group_by(gup_cytoband,gdw_cytoband,patient_id) %>% summarize(fusions = length(unique(patient_fusion))) %>% arrange(-fusions)

cnt_per_cytoband = cnt_per_cytoband %>% left_join(
  uq_fusions_hc %>% filter(variant_type=="low_af") %>%
  group_by(gup_cytoband,gdw_cytoband,patient_id) %>% summarize(low_af_fusions = length(unique(patient_fusion))) ) %>% 
  left_join(
  uq_fusions_hc %>% filter(variant_type=="tumor_specific") %>%
  group_by(gup_cytoband,gdw_cytoband,patient_id) %>% summarize(tumor_specific_fusions = length(unique(patient_fusion)))
)

cnt_per_chromosome  = uq_fusions_hc %>%  group_by(gup_chromosome,gdw_chromosome,patient_id) %>% summarize(fusions = length(unique(patient_fusion))) %>% arrange(-fusions)

cnt_per_chromosome = cnt_per_chromosome %>% left_join(
  uq_fusions_hc %>% filter(variant_type=="low_af") %>%
  group_by(gup_chromosome,gdw_chromosome,patient_id) %>% summarize(low_af_fusions = length(unique(patient_fusion))) ) %>% 
  left_join(
  uq_fusions_hc %>% filter(variant_type=="tumor_specific") %>%
  group_by(gup_chromosome,gdw_chromosome,patient_id) %>% summarize(tumor_specific_fusions = length(unique(patient_fusion)))
)


uq_fusions_hc %>% filter((patient_id=="PMCID203AAL"&grepl("chr12q",gup_cytoband))) %>%
  select(fusion_name,patient_id,variant_type,gup_cytoband,gdw_cytoband,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean) %>% 
  arrange(patient_id,variant_type,gup_cytoband,gdw_cytoband) %>% unique() 
                            
                            
uq_fusions_hc %>% filter((patient_id=="PMCID288AAK"&grepl("chr11",gup_cytoband))) %>%
  select(fusion_name,patient_id,variant_type,gup_cytoband,gdw_cytoband,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean) %>% 
  arrange(patient_id,variant_type,gup_cytoband,gdw_cytoband) %>% unique() 

## from chr12q 13-15 in patient PMCID203AAL and 2 from chr11q in PMCID288AAK, also note 2 from chr12q15 in PMCID288AAK with CN>3

```
PMCID288AAK  chr11q22.1  amplification between 4.8 and 7.2 with 6x low_af  and 3xtumor specific

PMCID203AAL       low_af     chr12q13-15 cn 4.6-6.2  wth 18 low af 

```{r}
## What if considering all CN>3?

cat(paste0("* All WGS validated ",print_fraction(uq_fusions_hc,"flag_in_cn_amplified"),"\n"))
cat(paste0("* tumor specific: ",print_fraction(filter(uq_fusions_hc,variant_type=="tumor_specific"),"flag_in_cn_amplified"),"\n"))
cat(paste0("* germline: ",print_fraction(filter(uq_fusions_hc,variant_type=="germline"),"flag_in_cn_amplified"),"\n"))
cat(paste0("* low-AF: ",print_fraction(filter(uq_fusions_hc,variant_type=="low_af"),"flag_in_cn_amplified"),"\n"))



low_af_selection %>% filter(gup_copy_ratio_l2fc_mean>1.58|gdw_copy_ratio_l2fc_mean>1.58) 
...
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_healthy_chimera"),"\n"))
...
anno_healthy_chimera_all
```{r}
cat(paste0("* All WGS validated ",print_fraction_uq_pairs(uq_gene_pairs_hc,"anno_healthy_chimera_all"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_healthy_chimera_all"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="germline"),"anno_healthy_chimera_all"),"\n"))
cat(paste0("* low-AF: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="low_af"),"anno_healthy_chimera_all"),"\n"))
cat(paste0("* ambiguous: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="ambiguous"),"anno_healthy_chimera_all"),"\n"))
...
cat(paste0("  * !patient_clinrel_fusion: ",print_fraction(filter(cohort_report,somatic_variant&!patient_clinrel_fusion),"anno_healthy_chimera"),"\n"))
cat(paste0("  *  onco/tsg & !patient_clinrel_fusion: ",print_fraction(filter(cohort_report,somatic_variant&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion),"anno_healthy_chimera"),"\n"))
...
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_cancer_chimera"),"\n"))
cat(paste0("  * tumor specific & clin-rel: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,clinically_validated&variant_type=="tumor_specific"),"anno_cancer_chimera"),"\n"))
cat(paste0("  * tumor specific & !clin-rel: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!clinically_validated&variant_type=="tumor_specific"),"anno_cancer_chimera"),"\n"))
...
uq_fusions_hc %>% filter(!patient_clinrel_fusion&variant_type=="tumor_specific"&anno_cancer_chimera) %>% select(fusion_display_properties,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean,gup_start_copy_ratio_l2fc,gdw_end_copy_ratio_l2fc) %>% left_join(patient_fusion_cnt_table[,c("patient_id","somatic_low_af_hc_cnt","fga","max_cna_fusion")])  %>% arrange(somatic_low_af_hc_cnt)
```
...
cat(paste0("  * !clinically relevant & precise + confident: ",print_fraction(filter(cohort_report,somatic_variant&precise_confident&!(clinically_validated|clinically_validated_reciprocal)),"anno_cancer_chimera"),"\n"))
cat(paste0("  * !patient_clinrel_fusion & precise + confident: ",print_fraction(filter(cohort_report,somatic_variant&precise_confident&!patient_clinrel_fusion),"anno_cancer_chimera"),"\n"))
cat(paste0("  * onco/tsg & !patient_clinrel_fusion & precise + confident: ",print_fraction(filter(cohort_report,somatic_variant&precise_confident&(anno_has_oncogene|anno_has_tsg)&!patient_clinrel_fusion),"anno_cancer_chimera"),"\n"))

cat(paste0("  * !patient_clinrel_fusion & precise + confident => Partner cancer chimera: ",print_fraction(filter(cohort_report,somatic_variant&precise_confident&!patient_clinrel_fusion),"anno_partner_cancer_chimera"),"\n"))
...
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_cancer_chimera & anno_healthy_chimera"),"\n"))
...
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"(anno_healthy_chimera|anno_sv_population)&anno_cancer_chimera"),"\n"))
...
*important , there are gl cancer chimera that are not in pop db or healthy chimera
```{r}
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="germline"),"!(anno_healthy_chimera|anno_sv_population)&anno_cancer_chimera"),"\n"))

filter(uq_fusions_hc,variant_type=="germline") %>% filter(!(anno_healthy_chimera|anno_sv_population)&anno_cancer_chimera)
...
Oncogene enrichment in tumor specific => ignore patient_clinrel_fusion

```{r  results="asis"}

cat(paste0("* All WGS validated ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion),"anno_has_oncogene"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
                                                           variant_type=="tumor_specific"),"anno_has_oncogene"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
                                                     variant_type=="germline"),"anno_has_oncogene"),"\n"))
cat(paste0("* low-AF: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
                                                   variant_type=="low_af"),"anno_has_oncogene"),"\n"))
cat(paste0("* ambiguous: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
                                                      variant_type=="ambiguous"),"anno_has_oncogene"),"\n"))


filter(uq_fusions_hc,!patient_clinrel_fusion&anno_has_tsg) %>% uq_uq_fusions()
filter(uq_fusions_hc,!patient_clinrel_fusion&anno_has_oncogene) %>% uq_uq_fusions()

filter(uq_fusions_hc,!patient_clinrel_fusion&anno_has_onco_or_tsg) %>% uq_uq_fusions()
...
cat(paste0("* All WGS validated ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion),"anno_has_tsg"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
                                                           variant_type=="tumor_specific"),"anno_has_tsg"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
...
```{r  results="asis"}
cat(paste0("* All predicted fusions ",print_fraction(filter(fusion_overview,!patient_clinrel_fusion),"anno_has_onco_or_tsg",remove_recurrent = T),"\n"))

cat(paste0("* All WGS validated ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="tumor_specific"),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="germline"),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* low-AF: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="low_af"),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* ambiguous: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="ambiguous"),"anno_has_onco_or_tsg"),"\n"))

filter(uq_gene_pairs_hc,!patient_clinrel_fusion&
                                                   variant_type=="low_af"&(anno_has_onco_or_tsg))
...
```{r  results="asis"}
cat(paste0("* All predicted fusions ",print_fraction(filter(fusion_overview,!patient_clinrel_fusion),"anno_cancer_gene_db",remove_recurrent = T),"\n"))

cat(paste0("* All WGS validated ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion),"anno_cancer_gene_db"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="tumor_specific"),"anno_cancer_gene_db"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="germline"),"anno_cancer_gene_db"),"\n"))
cat(paste0("* low-AF: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="low_af"),"anno_cancer_gene_db"),"\n"))
cat(paste0("* ambiguous: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="ambiguous"),"anno_cancer_gene_db"),"\n"))

filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="germline"&anno_cancer_gene_db)
filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="low_af"&anno_cancer_gene_db)
...
```{r}
filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="tumor_specific" & !(anno_has_oncogene |anno_has_tsg) & anno_cancer_gene_db)
```

Additional tumor specific onco/tsg but not in chimera db

```{r}
filter(uq_gene_pairs_hc,!patient_clinrel_fusion&variant_type=="tumor_specific" & (anno_has_oncogene |anno_has_tsg) & anno_cancer_chimera)
```
...
cat(paste0("* sanity check onco or tsg, high confidence tumor specific : ",
           filter(uq_gene_pairs_hc,!patient_clinrel_fusion&somatic_variant&(anno_has_oncogene|anno_has_tsg)) %>% nrow(),"\n"))
cat(paste0("* has oncogene : ",
           filter(uq_gene_pairs_hc,!patient_clinrel_fusion&somatic_variant&(anno_has_oncogene)) %>% nrow(),"\n"))
cat(paste0("* has tgs: ",
           filter(uq_gene_pairs_hc,!patient_clinrel_fusion&somatic_variant&(anno_has_tsg)) %>% nrow(),"\n"))
...
```{r results="asis"}
cat(paste0("* high conf tumor specific uq gene pairs: ",filter(uq_gene_pairs_hc,variant_type=="tumor_specific")%>% nrow(),"\n"))

cat(paste0("* all with oncogenes: ",filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_oncogene)%>% nrow(),"\n"))
cat(paste0("* ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_oncogene),"clinically_validated"),"\n"))
...
```{r results="asis"}
cat(paste0("* all with tsg: ",filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_tsg)%>% nrow(),"\n"))
cat(paste0("* ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_tsg),"clinically_validated"),"\n"))
```

Of all the high-conf tumor-specific gene pairs that contain tsgs & !oncogenes, what fraction is known clinically relevant?

```{r results="asis"}
cat(paste0("* all with tsg&!anno_has_oncogene: ",filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_tsg&!anno_has_oncogene)%>% nrow(),"\n"))
cat(paste0("* ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_tsg&!anno_has_oncogene),"clinically_validated"),"\n"))

filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&anno_has_tsg&!anno_has_oncogene&!clinically_validated) %>% select(fusion_name, patient_ids, anno_cancer_chimera)
```
...
```{r  results="asis"}
cat(paste0("* All WGS validated - fusion positve patient ",print_fraction_uq_pairs(filter(uq_gene_pairs_lc,patient_clinrel_fusion),"anno_has_onco_or_tsg"),"\n"))
```

Not fusion positive patient

```{r  results="asis"}
cat(paste0("* All WGS validated ",print_fraction_uq_pairs(filter(uq_gene_pairs_lc,!patient_clinrel_fusion),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_lc,!patient_clinrel_fusion&variant_type=="tumor_specific"),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_lc,!patient_clinrel_fusion&variant_type=="germline"),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* low-AF: ",print_fraction_uq_pairs(filter(uq_gene_pairs_lc,!patient_clinrel_fusion&variant_type=="low_af"),"anno_has_onco_or_tsg"),"\n"))
cat(paste0("* ambiguous: ",print_fraction_uq_pairs(filter(uq_gene_pairs_lc,!patient_clinrel_fusion&variant_type=="ambiguous"),"anno_has_onco_or_tsg"),"\n"))

filter(uq_gene_pairs_lc,!patient_clinrel_fusion&(anno_has_onco_or_tsg))
```
...
Conclusion: tumor specific not enriched in cancer chimera if you remove the patients with known driver fusion (= patient_clinrel_fusion)
...
fisher.table = matrix(c(
  nrow(uq_gene_pairs_hc %>% filter(variant_type=="tumor_specific" ) %>% filter(anno_has_tsg | anno_has_oncogene)),
  nrow(uq_gene_pairs_hc %>% filter(variant_type=="tumor_specific")  %>% filter(!anno_has_tsg & !anno_has_oncogene)),
uq_uq_fusions(fusion_overview %>% filter(!fusion_name %in% filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$fusion_name) %>% filter(anno_has_tsg | anno_has_oncogene) ),
uq_uq_fusions(fusion_overview %>% filter(!fusion_name %in% filter(uq_gene_pairs_hc,variant_type=="tumor_specific")$fusion_name) %>% filter(!anno_has_tsg & !anno_has_oncogene))
), nrow = 2)
f=fisher.test(fisher.table)
f
...
fisher.table = matrix(c(
  nrow(uq_gene_pairs_hc %>% filter(variant_type=="tumor_specific") %>% filter(anno_sv_population | anno_healthy_chimera)),
  nrow(uq_gene_pairs_hc %>% filter(variant_type=="tumor_specific") %>% filter(!anno_sv_population & !anno_healthy_chimera)),
...
```{r}
uq_gene_pairs_hc %>% filter(!patient_clinrel_fusion & (anno_has_onco_or_tsg | anno_cancer_chimera)) %>% select(fusion_name,variant_type,anno_cancer_chimera,anno_has_oncogene,anno_has_tsg) %>% arrange(variant_type,fusion_name) %>% unique() 

```

Incl also fusion positive patients as well
```{r}
uq_gene_pairs_hc %>% filter(variant_type=="tumor_specific" & (anno_has_onco_or_tsg | anno_cancer_chimera)) %>% select(fusion_name,variant_type,anno_cancer_chimera,anno_has_oncogene,anno_has_tsg) %>% arrange(variant_type,fusion_name) %>% unique() %>% nrow()
```

Somatic  chimera or onco or tsg & ! fusion positive patient
```{r}
uq_gene_pairs_hc %>% filter(!patient_clinrel_fusion & variant_type=="tumor_specific" & (anno_has_onco_or_tsg | anno_cancer_chimera)) %>% select(fusion_name,variant_type,anno_cancer_chimera,anno_has_oncogene,anno_has_tsg) %>% arrange(variant_type,fusion_name) %>% unique() %>% nrow()
```

Somatic chimera & ! fusion positive patient

```{r}
uq_gene_pairs_hc %>% filter(!patient_clinrel_fusion & variant_type=="tumor_specific" & anno_cancer_chimera) %>% select(fusion_name,variant_type,anno_cancer_chimera,anno_has_oncogene,anno_has_tsg) %>% arrange(variant_type,fusion_name) %>% unique() 
```
...
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_sv_population"),"\n"))
...
*Important SV population or healthy chimera
```{r results="asis"}
cat(paste0("* All WGS validated ",print_fraction_uq_pairs(uq_gene_pairs_hc,"anno_sv_population | anno_healthy_chimera"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_sv_population | anno_healthy_chimera"),"\n"))
...
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="germline"),"anno_healthy_chimera_all&anno_sv_population"),"\n"))
```
...
*Important
SV population or healthy chimera_all
```{r results="asis"}
cat(paste0("* All WGS validated ",print_fraction_uq_pairs(uq_gene_pairs_hc,"anno_sv_population | anno_healthy_chimera_all"),"\n"))
cat(paste0("* tumor specific: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_sv_population | anno_healthy_chimera_all"),"\n"))
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="germline"),"anno_sv_population | anno_healthy_chimera_all"),"\n"))
cat(paste0("* low-AF: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="low_af"),"anno_sv_population | anno_healthy_chimera_all"),"\n"))

```

SV population AND healthy chimera_all
```{r results="asis"}
cat(paste0("* germline: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="germline"),"anno_healthy_chimera_all&anno_sv_population"),"\n"))
```
...
uq_fusions_hc %>% filter(variant_type=="tumor_specific" & anno_sv_population) %>% select(fusion_name,patient_id,svtype,svlen,tumor_af_mean,normal_af_mean, repeat_family,segdup,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean,gup_start_copy_ratio_l2fc,gdw_end_copy_ratio_l2fc,anno_sv_db_nstd166,anno_sv_db_nstd186,anno_sv_db_dgv,anno_healthy_chimera) %>% arrange(fusion_name)
...
```{r results="asis"}
cat(paste0("* tumor-specific occur in normal population: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_healthy_chimera | anno_sv_population"),"\n"))

cat(paste0("* tumor-specific occur in normal population: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"anno_healthy_chimera_all | anno_sv_population"),"\n"))


cat(paste0("* tumor-specific NOT occur in normal population: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"!(anno_healthy_chimera | anno_sv_population)"),"\n"))

cat(paste0("* tumor-specific NOT occur in normal population: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"),"!(anno_healthy_chimera_all | anno_sv_population)"),"\n"))

cat(paste0("  * tumor-specific in total: ",nrow(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"))))

cat(paste0("  * excl fusion positive patients: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&!patient_clinrel_fusion),"anno_healthy_chimera | anno_sv_population"),"\n"))



cat(paste0("  * excl fusion positive patients: ",print_fraction_uq_pairs(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&!patient_clinrel_fusion),"!(anno_healthy_chimera | anno_sv_population)"),"\n"))

cat(paste0("  * tumor-specific excl fusion positive patients in total: ",nrow(filter(uq_gene_pairs_hc,variant_type=="tumor_specific"&!patient_clinrel_fusion))))
...
gdw_over = uq_fusions_hc %>% filter(somatic_variant ) %>% filter( (gdw_fpkm_zscore_supergroup >zfkpm_threshold) | (gdw_fpkm_zscore >zfkpm_threshold)) 
...
fisher.table = matrix(c(
uq_fusions(filter(uq_fusions_hc,somatic_variant&anno_gdw_oncogene&(gdw_fpkm_zscore_supergroup >zfkpm_threshold | gdw_fpkm_zscore >zfkpm_threshold))),
uq_fusions(filter(uq_fusions_hc,somatic_variant&anno_gdw_oncogene&!(gdw_fpkm_zscore_supergroup >zfkpm_threshold | gdw_fpkm_zscore >zfkpm_threshold))),
uq_fusions(filter(uq_fusions_hc,somatic_variant&!anno_gdw_oncogene&(gdw_fpkm_zscore_supergroup >zfkpm_threshold | gdw_fpkm_zscore >zfkpm_threshold))),
uq_fusions(filter(uq_fusions_hc,somatic_variant&!anno_gdw_oncogene&!(gdw_fpkm_zscore_supergroup >zfkpm_threshold | gdw_fpkm_zscore >zfkpm_threshold)))
), nrow = 2)
...
```{r}
mean(filter(uq_fusions_hc,somatic_variant&anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,na.rm=T)
mean(filter(uq_fusions_hc,somatic_variant&!anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,na.rm=T)

wilcox.test(filter(uq_fusions_hc,somatic_variant&anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,filter(uq_fusions_hc,somatic_variant&!anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,alternative = "greater")
...
print("all onco")
mean(filter(genes_long_selection,grepl("oncogene",label))$fpkm_zscore_supergroup,na.rm=T)
mean(filter(genes_long_selection,!grepl("oncogene",label))$fpkm_zscore_supergroup,na.rm=T)

wilcox.test(filter(genes_long_selection,grepl("oncogene",label))$fpkm_zscore_supergroup,
            filter(genes_long_selection,!grepl("oncogene",label))$fpkm_zscore_supergroup)


print("onco downstream ")
mean(filter(genes_long_selection,side=="downstream"&grepl("oncogene",label))$fpkm_zscore_supergroup,na.rm=T)
mean(filter(genes_long_selection,side=="downstream"&!grepl("oncogene",label))$fpkm_zscore_supergroup,na.rm=T)

wilcox.test(filter(genes_long_selection,side=="downstream"&grepl("oncogene",label))$fpkm_zscore_supergroup,
            filter(genes_long_selection,side=="downstream"&!grepl("oncogene",label))$fpkm_zscore_supergroup)

print("TSG")
mean(filter(genes_long_selection,grepl("tsg",label))$fpkm_zscore_supergroup,na.rm=T)
mean(filter(genes_long_selection,!grepl("tsg",label))$fpkm_zscore_supergroup,na.rm=T)

wilcox.test(filter(genes_long_selection,grepl("tsg",label))$fpkm_zscore_supergroup,
            filter(genes_long_selection,!grepl("tsg",label))$fpkm_zscore_supergroup)

```

Exactly the same but without patients with clin rel fusions
```{r}
genes_long_selection = filter(genes_long,patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant_only)$patient_fusion_sv & !patient_clinrel_fusion)
...
mean(filter(genes_long_selection, grepl("tsg",label) & !grepl("onco",label))$fpkm_zscore_supergroup,na.rm=T)
mean(filter(genes_long_selection, !grepl("tsg",label) | grepl("onco",label))$fpkm_zscore_supergroup,na.rm=T)

wilcox.test(filter(genes_long_selection, grepl("tsg",label) & !grepl("onco",label))$fpkm_zscore_supergroup,
            filter(genes_long_selection, !grepl("tsg",label) | grepl("onco",label))$fpkm_zscore_supergroup)
...
over_exp = genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold))
...
over_exp_onco=genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv)  %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )& grepl("oncogene",label))
...
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion))
print("all over expressed")
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold)))

print("over expressed oncogenes")
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )& grepl("oncogene",label))

print("Fraction of oncogenes that is over expressed")
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )& grepl("oncogene",label)))/
  uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion) %>% filter(grepl("oncogene",label)))

print("Fraction of over expr genes that is onco gene")
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )& grepl("oncogene",label)))/uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )))
...
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion ) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc>cna_threshold) %>% filter(grepl("oncogene",label)) %>% select(patient_fusion) %>% unique()
...
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc>cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow()
print("cna <= cna_threshold")
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc<=cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow()

print("Fraction of >cna_threshold genes that is >zfkpm_threshold fpkm")
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc>cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow() / genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter( cna_l2fc>cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow()  
...
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc>cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow() / genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold)) %>% select(patient_fusion) %>% unique() %>% nrow()  
...
```{r}
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter(!(fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc>cna_threshold) %>% select(patient_fusion, fpkm_zscore_supergroup, fpkm_zscore,cna_l2fc,gene_name) %>% unique()
```

Wilcoxon test for mean zfpkm domain of CN > 3x vs not 
```{r}

mean(filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc>cna_threshold)$fpkm_zscore_supergroup,na.rm=T)
mean(filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc<=cna_threshold)$fpkm_zscore_supergroup,na.rm=T)


wilcox.test( 
  filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc>cna_threshold)$fpkm_zscore_supergroup,
  filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc<=cna_threshold)$fpkm_zscore_supergroup)
...
mean(filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc>cna_threshold & grepl("oncogene",label))$fpkm_zscore_supergroup,na.rm=T)

mean(filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc>cna_threshold & !grepl("oncogene",label))$fpkm_zscore_supergroup,na.rm=T)

wilcox.test( 
  filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc>cna_threshold & grepl("oncogene",label))$fpkm_zscore_supergroup,
  filter(genes_long, patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv & cna_l2fc>cna_threshold & !grepl("oncogene",label))$fpkm_zscore_supergroup)
...
fisher.table = matrix(c(
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,somatic_variant)$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & grepl("oncogene",label)) %>% select(patient_fusion) %>% unique() %>% nrow(),
...
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion & patient_id %in% patients_high_fusion_burden) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold)) %>% select(patient_fusion) %>% unique() %>% nrow()
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion & patient_id %in% patients_high_fusion_burden) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc>cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow()
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion & patient_id %in% patients_high_fusion_burden) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & cna_l2fc<=cna_threshold) %>% select(patient_fusion) %>% unique() %>% nrow()
...
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup<(-zfkpm_threshold) | fpkm_zscore <(-zfkpm_threshold) )& grepl("tsg",label)))/
  uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion) %>% filter(grepl("tsg",label)))

print("Fraction of downregulated genes that is tsg")
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup<(-zfkpm_threshold) | fpkm_zscore <(-zfkpm_threshold) )& grepl("tsg",label)))/uq_fusions(genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,!germline_variant)$patient_fusion_sv & !patient_clinrel_fusion)  %>% filter((fpkm_zscore_supergroup<(-zfkpm_threshold) | fpkm_zscore <(-zfkpm_threshold) )))
...
print("mean fpkm of supergroup, excl TP53- fusion patients")
mean(filter(gene_expression_analysis,ensembl_id %in% candidate$gup_ensembl_id & !patient_id %in% candidate$patient_id & supergroup_label %in% candidate$supergroup_label)$fpkm_log)
...
wilcox.test( filter(gene_expression_analysis,ensembl_id %in% candidate$gup_ensembl_id & patient_id %in% candidate$patient_id)$fpkm_log, 
             filter(gene_expression_analysis,ensembl_id %in% candidate$gup_ensembl_id & !patient_id %in% candidate$patient_id & supergroup_label %in% candidate$supergroup_label)$fpkm_log)
```
...
genes_long %>% filter(patient_fusion_sv %in% uq_fusions_hc$patient_fusion_sv) %>% filter(fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold) %>% select(patient_fusion) %>% unique() %>% nrow()

print("Fraction of oncogenes that is over expressed")
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% uq_fusions_hc$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )& grepl("oncogene",label)))/uq_fusions(genes_long %>% filter(patient_fusion_sv %in% uq_fusions_hc$patient_fusion_sv) %>% filter(grepl("oncogene",label)))

print("Fraction of over expr genes that is onco gene")
uq_fusions(genes_long %>% filter(patient_fusion_sv %in% uq_fusions_hc$patient_fusion_sv) %>% filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold )& grepl("oncogene",label)))/uq_fusions(genes_long %>% filter(patient_fusion_sv %in% uq_fusions_hc$patient_fusion_sv) %>% filter(fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore >zfkpm_threshold ))


genes_long %>% filter(patient_fusion_sv %in% uq_fusions_hc$patient_fusion_sv)  %>%  filter(fpkm_zscore_supergroup<=zfkpm_threshold & fpkm_zscore<=zfkpm_threshold & grepl("oncogene",label) & !grepl("tsg",label)) %>% select(patient_fusion) 
...
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,clinically_validated|clinically_validated_reciprocal)$patient_fusion_sv )  %>%  filter((fpkm_zscore_supergroup>zfkpm_threshold | fpkm_zscore>zfkpm_threshold) & grepl("oncogene",label))

## oncogenes and fpkm <1.9
genes_long %>% filter(patient_fusion_sv %in% filter(uq_fusions_hc,clinically_validated|clinically_validated_reciprocal)$patient_fusion_sv )  %>%  filter(fpkm_zscore_supergroup<1.9 & fpkm_zscore<1.9 & grepl("oncogene",label)) %>% select(patient_fusion) %>% unique() %>% nrow()
...
SV types of candidate fusions 
```{r}
selected_fusion_candidates =uq_fusions_df %>% filter(rna_consensus==T) %>%
  ...
  filter(!patient_clinrel_fusion & (anno_has_kinase|anno_has_onco_or_tsg))


selected_fusion_candidates = selected_fusion_candidates %>% left_join(patient_labels_lookup) 

summary(selected_fusion_candidates %>%  select(patient_fusion,svtype_label) %>% unique())
summary(selected_fusion_candidates %>%  select(fusion_name,svtype_label) %>% unique())

uq_gene_pairs_df %>% filter(fusion_name %in% selected_fusion_candidates$fusion_name) %>% select(fusion_name,svtype_label)


selected_fusion_candidates %>% select(patient_fusion,variant_type,svtype_label,tumor_af_mean,tumor_type_label) %>% arrange(patient_fusion) %>% unique()
summary(selected_fusion_candidates %>% filter(somatic_variant) %>% select(patient_fusion,svtype_label) %>% unique())
summary(selected_fusion_candidates %>% filter(somatic_variant) %>% select(fusion_name,svtype_label) %>% unique())

  
export_selected_fusion_candidates = selected_fusion_candidates %>% select(patient_id,patient_label,patient_fusion,variant_type,svtype_label,tumor_af_mean,gup_label,gdw_label,tumor_type_label,gup_fpkm_zscore_supergroup,gdw_fpkm_zscore_supergroup,gup_fpkm_supergroup_pval,gdw_fpkm_supergroup_pval,gup_start_copy_ratio_l2fc,gdw_end_copy_ratio_l2fc,gup_sv_merged_coordinate,gdw_sv_merged_coordinate) %>% arrange(patient_fusion) %>% unique() 

#export_selected_fusion_candidates %>% View()

#write.table(export_selected_fusion_candidates ,paste0(reports_dir,"selected_fusion_candidates.tsv"),col.names=T,quote=T,row.names = F,sep="\t",na="")
...
genes_long %>% filter(patient_fusion_sv %in% selected_fusion_candidates$patient_fusion_sv) %>% filter(grepl("oncogene",label)) %>%
  filter(fpkm_zscore_supergroup>zfkpm_threshold) %>% select(patient_id,patient_label,fusion_name,gene_name,fpkm_zscore_supergroup,fpkm_supergroup_pval)

print("not in domain but on group/cohort")
genes_long %>% filter(patient_fusion_sv %in% selected_fusion_candidates$patient_fusion_sv) %>% filter(grepl("oncogene",label)) %>%
  filter(fpkm_zscore_supergroup<zfkpm_threshold & (fpkm_zscore_group>zfkpm_threshold | fpkm_zscore >zfkpm_threshold)) %>% select(patient_id,patient_label,fusion_name,gene_name,fpkm_zscore_group,fpkm_zscore,fpkm_pval)
...
genes_long %>% filter(patient_fusion_sv %in% selected_fusion_candidates$patient_fusion_sv) %>% filter(grepl("tsg",label)&!grepl("oncogene",label)) %>%
  filter(fpkm_zscore_supergroup>zfkpm_threshold) 
...
## Fusions with cancer related genes non-onco/tsg - incl kinases


```{r}

selected_fusion_candidates_larger =uq_fusions_df %>% filter(rna_consensus==T) %>%
  filter(((low_af|somatic_variant)&high_confidence)|
           (somatic_variant & ( (grepl(",",tools_any_wgs_fusioncatcher)&gup_location_fusioncatcher=="composite") | 
                                  (grepl(",",tools_any_wgs_starfusion)&gup_location_starfusion=="composite")))) %>%
  filter(!patient_clinrel_fusion & (anno_has_kinase|anno_cancer_gene_db|anno_has_onco_or_tsg))

```
Kinases
```{r}

selected_fusion_candidates_larger %>% filter(grepl("kinase",gup_label) | grepl("kinase",gdw_label))  %>%
   left_join(patient_labels_lookup)  %>% select(patient_id,patient_label,patient_fusion,variant_type,svtype_label,tumor_af_mean,gup_label,gdw_label,tumor_type_label,gup_fpkm_zscore_supergroup,gdw_fpkm_zscore_supergroup,gup_fpkm_supergroup_pval,gdw_fpkm_supergroup_pval,gup_start_copy_ratio_l2fc,gdw_end_copy_ratio_l2fc) %>% arrange(patient_fusion) %>% unique() #%>% View()
...
Upregulated:
```{r}
genes_long %>% filter(patient_fusion_sv %in% selected_fusion_candidates_larger$patient_fusion_sv) %>% filter(grepl("kinase",label)) %>%
  filter(fpkm_zscore_supergroup>zfkpm_threshold) %>% select(patient_id,patient_label,fusion_name,gene_name,fpkm_zscore_supergroup,fpkm_supergroup_pval)
...
fusions w/o onco/tsg but cancer genes:
```{r}
selected_fusion_candidates_larger %>% filter(!(anno_has_onco_or_tsg)&anno_cancer_gene_db) %>% select(patient_fusion) %>%  unique()
```
...
```{r}
selected_fusion_candidates_larger %>% filter(!(anno_has_onco_or_tsg)&!anno_cancer_gene_db) %>% select(patient_fusion,anno_healthy_chimera,anno_sv_population,repeat_family,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean) %>%  unique()
```
...
gup_over = uq_fusions_df %>% filter(somatic_variant & !patient_clinrel_fusion) %>% filter( (gup_fpkm_zscore_supergroup >zfkpm_threshold | gup_fpkm_zscore >zfkpm_threshold | gup_fpkm_zscore_group >zfkpm_threshold)) 
gup_under = uq_fusions_df  %>% filter(somatic_variant & !patient_clinrel_fusion) %>% filter( (gup_fpkm_zscore_supergroup < (-zfkpm_threshold) | gup_fpkm_zscore < (-zfkpm_threshold) | gup_fpkm_zscore_group < (-zfkpm_threshold))) 
...
mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup),na.rm=T)
mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & !anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup),na.rm=T)

wilcox.test( filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,
             filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & !anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,alternative = "greater")

## slightly but NS

mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & anno_gdw_tsg)$gdw_fpkm_zscore_supergroup),na.rm=T)
mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & !anno_gdw_tsg)$gdw_fpkm_zscore_supergroup),na.rm=T)

mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & anno_gup_tsg)$gup_fpkm_zscore_supergroup),na.rm=T)
mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & !anno_gup_tsg)$gup_fpkm_zscore_supergroup),na.rm=T)


 ## same for cancer related genes???

filter(uq_fusions_df,somatic_variant & (clinically_validated | clinically_validated_reciprocal)  & anno_gdw_oncogene) %>% select(fusion_name,gdw_fpkm_zscore_supergroup)
mean(abs(filter(uq_fusions_df,somatic_variant & !patient_clinrel_fusion & !anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup),na.rm=T)
...
```{r}
cohort_report %>% filter(!patient_clinrel_fusion & somatic_variant & precise_confident
                         & (anno_gup_oncogene & (gup_fpkm_zscore_supergroup >zfkpm_threshold | gup_fpkm_zscore >zfkpm_threshold))) %>%  select(fusion_display_properties,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean,gup_start_copy_ratio_l2fc,gdw_end_copy_ratio_l2fc,contains("gup_fpkm_zscore"),contains("gup_fpkm_log_normal_dist"),contains("tx_gup_exon_l2fc"),tx_gup_involved_fraction) %>% unique()

cohort_report %>% filter(!patient_clinrel_fusion & somatic_variant & precise_confident
                         & (anno_gdw_oncogene & (gdw_fpkm_zscore_supergroup >zfkpm_threshold | gdw_fpkm_zscore >zfkpm_threshold))) %>%
...
Domain:
```{r}
mean(filter(cohort_report,somatic_variant & precise_confident & anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,na.rm=T)
mean(filter(cohort_report,somatic_variant & precise_confident & !anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,na.rm=T)

mean(filter(cohort_report,!patient_clinrel_fusion &somatic_variant & precise_confident & anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,na.rm=T)
mean(filter(cohort_report,!patient_clinrel_fusion &somatic_variant & precise_confident & !anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,na.rm=T)

wilcox.test( filter(cohort_report,!patient_clinrel_fusion & somatic_variant & precise_confident & anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,
             filter(cohort_report,!patient_clinrel_fusion & somatic_variant & precise_confident & !anno_gdw_oncogene)$gdw_fpkm_zscore_supergroup,alternative = "greater")
```
...
mean(filter(cohort_report,!patient_clinrel_fusion &somatic_variant & precise_confident & anno_gdw_oncogene)$gdw_fpkm_zscore_group,na.rm=T)
mean(filter(cohort_report,!patient_clinrel_fusion &somatic_variant & precise_confident & !anno_gdw_oncogene)$gdw_fpkm_zscore_group,na.rm=T)

wilcox.test( filter(cohort_report,!patient_clinrel_fusion & somatic_variant & precise_confident & anno_gdw_oncogene)$gdw_fpkm_zscore_group,
             filter(cohort_report,!patient_clinrel_fusion & somatic_variant & precise_confident & !anno_gdw_oncogene)$gdw_fpkm_zscore_group,alternative = "greater")
```
...
cohort_report %>% filter(somatic_variant & precise_confident
                         & (anno_gup_tsg & (gup_fpkm_zscore_supergroup < -1.9 | gup_fpkm_zscore < -1.9))) %>% 
...
cohort_report %>% filter(somatic_variant & precise_confident
                         & (anno_gdw_tsg & (gdw_fpkm_zscore_supergroup < -1.9 | gdw_fpkm_zscore < -1.9))) %>%
select(fusion_display_properties,gup_copy_ratio_l2fc_mean,gdw_copy_ratio_l2fc_mean,gup_start_copy_ratio_l2fc,gdw_end_copy_ratio_l2fc,contains("gdw_fpkm_zscore"),contains("gdw_fpkm_log_normal_dist"),contains("tx_gdw_exon_l2fc"),tx_gdw_involved_fraction) %>% unique()
```
...
```{r}
cohort_report %>% filter(somatic_variant&precise_confident) %>% 
  filter( ((anno_gup_oncogene|anno_gup_tsg) &  tx_gup_exon_l2fc_sign_cohort)  ) %>% mutate(side="upstream") %>% select(side,fusion_name,patient_clinrel_fusion,contains("tx_gup_exon_l2fc"),contains("gup_fpkm_zscore"),anno_cancer_chimera) %>% arrange(-patient_clinrel_fusion,fusion_name)

cohort_report %>% filter(somatic_variant&precise_confident) %>% 
  filter(     ((anno_gdw_oncogene|anno_gdw_tsg) & tx_gdw_exon_l2fc_sign_cohort) ) %>% mutate(side="downstream") %>% select(side,fusion_name,patient_clinrel_fusion,contains("tx_gdw_exon_l2fc"),contains("gdw_fpkm_zscore"),anno_cancer_chimera) %>% arrange(-patient_clinrel_fusion,fusion_name)
...
cohort_report %>% filter(somatic_variant&precise_confident) %>% 
  filter( (anno_gup_oncogene|anno_gup_tsg) &  (!tx_gup_exon_l2fc_sign_cohort & tx_gup_exon_l2fc_sign_cohort_close)  ) %>% mutate(side="upstream") %>% select(side,fusion_name,patient_clinrel_fusion,contains("tx_gup_exon_l2fc"),contains("gup_fpkm_zscore"),anno_cancer_chimera,anno_gup_oncogene,anno_gup_tsg) %>% arrange(-patient_clinrel_fusion,fusion_name)


cohort_report %>% filter(somatic_variant&precise_confident) %>% 
  filter((anno_gdw_oncogene|anno_gdw_tsg) & (!tx_gdw_exon_l2fc_sign_cohort & tx_gdw_exon_l2fc_sign_cohort_close) ) %>% mutate(side="downstream") %>% select(side,fusion_name,patient_clinrel_fusion,contains("tx_gdw_exon_l2fc"),contains("gdw_fpkm_zscore"),anno_cancer_chimera,anno_gdw_oncogene,anno_gdw_tsg) %>% arrange(-patient_clinrel_fusion,fusion_name)
