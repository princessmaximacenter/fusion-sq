...
## Last update: 2021-08-25 (refactoring, test data)
## 
...
fusion_overview_anno_path = paste0(reports_dir,fusion_overview_anno_outfile)


## START: read in and start annotation
...
fusion_overview = fusion_overview %>%
  left_join(patient_metadata %>% select(patient_id, tumor_type_label, primary_group, primary_group_label, 
                                        supergroup, supergroup_label,primary_group_shorthand_label), 
            by="patient_id") 

cohort_fusion_level_results = cohort_fusion_level_results %>% filter(patient_id %in% patient_metadata$patient_id)
cohort_results = cohort_results %>% filter(patient_id %in% patient_metadata$patient_id)
fusion_overview = fusion_overview  %>% filter(patient_id %in% patient_metadata$patient_id)
...
### SV level properties
## supporting SVs with gene annotation
supporting_sv_anno_df = read.table(cohort_supporting_sv_anno_path,sep = "\t",header=T)
...
fusion_overview = fusion_overview %>% mutate(anno_in_any_cancer = 
                                               (anno_partner_cancer_chimera | anno_cancer_gene_db |
                                                  anno_starfusion_cancer_db  | anno_grobner_druggable))




## Annotate clinically_relevant fusions if annotation is present
if("fusion" %in% names(patient_metadata)) {
  #fusion == true 
  fusion_overview = fusion_overview %>% mutate(patient_clinrel_fusion = patient_id %in% filter(patient_metadata,fusion_status)$patient_id)
  
  for(id in filter(patient_metadata,fusion_status)$patient_identifier) {
    patient = filter(patient_metadata,patient_identifier==id)
    
    fusion_overview = fusion_overview[fusion_overview$patient_id==patient$patient_id,] %>% dplyr::mutate(clinically_validated = 
                                                                                                           (gup_gene_id == patient$left_gene & gdw_gene_id == patient$right_gene))
    fusion_overview = fusion_overview[fusion_overview$patient_id==patient$patient_id,] %>% dplyr::mutate(clinically_validated_reciprocal = 
                                                                                                           (gdw_gene_id == patient$left_gene & gup_gene_id == patient$right_gene))
...
cohort_report = cohort_report %>%
  left_join( patient_metadata %>% select(patient_id,tumor_id,normal_id,rna_id), by="patient_id")
