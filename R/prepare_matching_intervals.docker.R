...
##START
  ...
  for(i in 1:nrow(fusion_anno_table)){
    ...
    partner_gene_gup$sf_breakpoint = as.character(partner_gene_gup$sf_breakpoint)#in case not read as string
    ...
    partner_gene_gdw$sf_breakpoint = as.character(partner_gene_gdw$sf_breakpoint) #in case not read as string
