...
make_fusion_anno_table_fc = function(fc_file,genes,patient) {
  ...
  names(fusion_anno_table)=colnames
  
  fusion_anno_table$identifier  = as.integer(rownames(fusion_anno_table)) #to match with fusion object generated by chimeraviz
  
  fusion_anno_table = fusion_anno_table %>% 
    rename(gup_ensembl_id = Gene_1_id5end_fusion_partner, gdw_ensembl_id = Gene_2_id3end_fusion_partner) %>% 
    rename(gup_sf_breakpoint = Fusion_point_for_gene_15end_fusion_partner, gdw_sf_breakpoint=Fusion_point_for_gene_23end_fusion_partner) %>% 
    rename(gup_sf_exon_id = Exon_1_id5end_fusion_partner, gdw_sf_exon_id = Exon_2_id3end_fusion_partner) %>%
    rename(gup_gene_id = Gene_1_symbol5end_fusion_partner, gdw_gene_id = Gene_2_symbol3end_fusion_partner) %>% 
    rename(predicted_effect=Predicted_effect) %>%
    rename(annots=Fusion_description)
  
  fusion_anno_table$gup_sf_breakpoint = paste0("chr",fusion_anno_table$gup_sf_breakpoint)
  ...
  #remove mitochondrial 
  fusion_anno_table=fusion_anno_table[!(grepl("chrM",fusion_anno_table$gup_sf_breakpoint)|grepl("chrM",fusion_anno_table$gdw_sf_breakpoint)),]
  
  #add versioned id and gene type based on genes df
  fusion_anno_table = fusion_anno_table %>%
    left_join(genes[,c("ensembl_id","gene_id","gene_type")],by=c("gup_ensembl_id"="ensembl_id")) %>% 
    rename(gup_gene_type=gene_type, gup_ensembl_version=gene_id) %>%
    left_join(genes[,c("ensembl_id","gene_id","gene_type")],by=c("gdw_ensembl_id"="ensembl_id")) %>% 
    rename(gdw_gene_type=gene_type, gdw_ensembl_version=gene_id) 
  
  # annotate fusion bp distance
  fusion_anno_table$fusion_bp_distance = mapply(get_fusion_bp_distance, fusion_anno_table$gup_sf_breakpoint, fusion_anno_table$gdw_sf_breakpoint)
  
  #all annots: fusion_anno_table$annots %>% strsplit(split=",",fixed=T) %>% unlist() %>% unique()
  
  # Healthy chimera and cancer associated
  fusion_anno_table = fusion_anno_table %>% mutate(anno_healthy_chimera = 
                                                     grepl("gtex|healthy|banned",annots))
  
  fusion_anno_table = fusion_anno_table %>% mutate(anno_fusioncatcher_cancer= 
                                                     grepl("oncogene|cancer|chimer|tcga|tumor",annots))
